  ...
  <a-card :body-style="{ padding: 0 }" :bordered="false">
    <div class="vaw-tab-bar-container">
      <div class="flex items-center h-full">
        ...
        <a-scrollbar
          ref="scrollbar"
          outer-style="position: relative; height: 100%; overflow: hidden; flex: 1 1 0%; display: flex; align-items: center;"
          style="width: 100%; height: 100%; overflow-x: auto; display: flex; align-items: center"
        >
          <div class="flex">
            <a-button
      ...
      <ul v-show="showContextMenu" class="contex-menu-wrapper" :style="contextMenuStyle">
        ...
        <li>
          <a-button type="text" @click="closeOther">
            <template #icon>
              <ArrowSwap style="vertical-align: text-bottom" />
            </template>
            关闭其他
          </a-button>
        </li>
...
<script lang="ts" name="TabBar">
  ...
  export default defineComponent({
    ...
    mounted() {
      // 存储当前组件实例，为了在其他组件或方法中能便捷获取到该实例
      const globalRefsStore = useGlobalRefsStore()
      globalRefsStore.setTabBarRef(this)
    },
    methods: {
      ...
      refreshRoute() {
        this.deleteRefreshCachePath(this.$route as any).then(() => {
      ...
      leftArrowClick() {
        const scrollbar = this.$refs.scrollbar as any
        const scrollX = scrollbar.$el?.querySelector('.arco-scrollbar-container')?.scrollLeft || 0
        scrollbar.scrollTo({

<template>
  <div
    ...
    :class="[
      !appStore.isCollapse ? 'open-status' : 'close-status',
      bgColor,
      { 'standard-blue-bar-wrapper': appStore.isStandardTheme }
    ]"
  >
    <div
      class="tab-split-tab-wrapper"
      :class="{ 'standard-blue-tab-wrapper': appStore.isStandardTheme }"
    >
      <Logo class="tab-split-logo-wrapper" :show-title="false" />
      <a-scrollbar
        outer-style="height: calc(100% - 48px) ; overflow: hidden"
...
<script lang="ts" name="TabSplitSideBar">
    ...
    setup() {
      ...
      function findPath(item: SplitTab) {
        if (item.children && item.children.length > 0) {
          ...
          } else if (isExternal(firstItem.path as string)) {
            routes.length = 0
...
<style scoped lang="less">
  ...
  .sidebar-bg-dark {
    ...
    .tab-split-tab-wrapper {
      ...
      &.standard-blue-tab-wrapper {
        background-color: var(--standard-dark-blue);
      }
      .label-item-wrapper {
        color: #fff;
      }

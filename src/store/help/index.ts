...
import useTHeaderKeysStore from '../modules/theader-keys'
...
export function getFilePath(it: OriginRoute) {
  if (!it.componentPath) {
    it.componentPath = it.route
  }
  it.componentPath = resolve('/', it.componentPath)
  return '/src/views' + it.componentPath + '.vue'
}
...
export function filterRoutesFromLocalRoutes(
  ...
  const filterRoute = localRoutes.find(it => {
    return resolve(path, it.path) === route.route
  })
  if (filterRoute) {
    filterRoute.meta = {
      title: route.name,
      affix: !!route.affix,
      cacheable: !!route.cached,
...
// 根目录路由，需要以'/'开头，若不是，则加上
export function handleRouteStartsWith(routes: Array<OriginRoute>): Array<OriginRoute> {
  routes.map(item => {
...
// 保存menu接口中route对应的tableName字段
export function handleTHeaderKeysByRemoteRoutes(res: Array<OriginRoute>) {
  const currentRoutes = JSON.parse(JSON.stringify(res))
  const theaderKeysStore = useTHeaderKeysStore()
  function handleRoute(routes: any, parentPath: string) {
    routes.forEach((it: OriginRoute) => {
      if (!isExternal(it.route)) {
        it.route = resolve(parentPath, it.route)
        // 最后一级路由，才保存tableName
        if (!it.children || !it.children.length) {
          theaderKeysStore.saveKey({
            key: it.route,
            value: it.tableName || ''
          })
        }
      ...
      if (it.children && it.children.length > 0) {
        handleRoute(it.children, it.route)
      }
    })
  }
  handleRoute(currentRoutes, '/')
}

// 保存各详情页面的route对应的 tableName 字段
export function handleTHeaderKeysByLocalRoutes() {
  const theaderKeysStore = useTHeaderKeysStore()
  const routes = extraRoutes.map(item => item.children ?? []).flat()
  routes.forEach((item: RouteRecordRaw & { meta?: { title?: string; tableName?: string } }) => {
    if (item?.meta?.tableName) {
      theaderKeysStore.saveKey({
        key: item.path,
...
export function generatorRoutes(res: Array<OriginRoute>): any {
  const tempRoutes: Array<RouteRecordRaw> = []
  res.forEach(it => {
    // 根据是否有子级，判断是否为菜单
    const isMenuFlag = isMenu(it)
    const localRoute = isMenuFlag ? filterRoutesFromLocalRoutes(it, defaultRoutes) : null
    if (localRoute) {
      tempRoutes.push(localRoute as RouteRecordRaw)
    } else {
      const route: RouteRecordRaw = {
        path: it.externalLink && isExternal(it.externalLink) ? it.externalLink : it.route,
        name: it.routeName || getNameByUrl(it.route),
